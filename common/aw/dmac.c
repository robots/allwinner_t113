...
{
	//uart_printf("dmac start %08x\n", desc);
	//uart_printf("desc %08x %08x\n", desc->src_addr, desc->nbytes);
	if ((uint32_t)desc & 3) {
...
//	uart_printf("dmac irq %08x %08x\n", pend0, pend1);

	for (uint32_t i = 0; i < 7; i++) {
		uint32_t mask = 0x7 << (i*4);
		if (pend0 & mask) {
			if (dmac_handlers[i].fnc) {
//				uart_printf("handler %d\n", i);
